...
def generate_game_title(console):
    ...
    return f"{random.choice(prefixes)} {random.choice(suffixes)} {console}"
...
for console in consoles:
    filename = console.lower().replace(' ', '').replace('/', '') + '.html'
...
''')
        for i in range(30):
            title = generate_game_title(console)
            rating = random.randint(1, 10)
            review = generate_review()
            f.write(f'''        <div class="review-card">
            <h3>{title}</h3>
